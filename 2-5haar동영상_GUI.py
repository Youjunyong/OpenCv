...
def selectFile():
    file_name = filedialog.askopenfilename(initialdir="Opencv_practice/video",
...
def detectAndDisplay():
    ...
    cv2image = cv2.cvtColor(frame, cv2.COLOR_BGR2LUV)
...
Button(main, text="File Select", height=2, command=lambda: selectFile()).grid(row=1,
 column=0, columnspan=4, sticky=(W, E))
